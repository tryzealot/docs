...
所有用户必须立即升级到 `6.2.1` 或更高版本。之前的版本存在高危 GraphQL 安全问题，影响认证与 Pundit 授权，包括未携带 Bearer Token 也可执行查询、`User` 敏感字段泄露，以及缺失资源授权检查。
...
- `Web` 新增全新的通知机制、确认窗口样式 [#2110](https://github.com/tryzealot/zealot/pull/2110) [#2116](https://github.com/tryzealot/zealot/pull/2116)
- `Web` 新增免密登录认证 [#2118](https://github.com/tryzealot/zealot/pull/2118)
- `Web` 上传应用没有存储空间新增友好提示（替代错误页面）
...
- `Web` 修复文档里 API 路径的错误
- `Web` 修复系统备份时有一定概率执行失败
- `Web` 优化很多页面的样式统一，黑暗模式的适配修复
- `Web` 优化中英文语言包的问题
